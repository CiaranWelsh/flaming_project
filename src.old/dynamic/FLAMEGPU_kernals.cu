...
__constant__ int d_xmachine_memory_fibril_count;

/* Agent state count constants */

__constant__ int d_xmachine_memory_fibril_default_count;
...
////////////////////////////////////////////////////////////////////////////////////////////////////////
/* Dynamically created fibril agent functions */

/** reset_fibril_scan_input
 * fibril agent reset scan input function
 * @param agents The xmachine_memory_fibril_list agent list
 */
__global__ void reset_fibril_scan_input(xmachine_memory_fibril_list* agents){
...
/** scatter_fibril_Agents
 * fibril scatter agents function (used after agent birth/death)
 * @param agents_dst xmachine_memory_fibril_list agent list destination
 * @param agents_src xmachine_memory_fibril_list agent list source
 * @param dst_agent_count index to start scattering agents from
 */
__global__ void scatter_fibril_Agents(xmachine_memory_fibril_list* agents_dst, xmachine_memory_fibril_list* agents_src, int dst_agent_count, int number_to_scatter){
	//global thread index
	int index = (blockIdx.x*blockDim.x) + threadIdx.x;
...
/** append_fibril_Agents
 * fibril scatter agents function (used after agent birth/death)
 * @param agents_dst xmachine_memory_fibril_list agent list destination
 * @param agents_src xmachine_memory_fibril_list agent list source
 * @param dst_agent_count index to start scattering agents from
 */
__global__ void append_fibril_Agents(xmachine_memory_fibril_list* agents_dst, xmachine_memory_fibril_list* agents_src, int dst_agent_count, int number_to_append){
	//global thread index
	int index = (blockIdx.x*blockDim.x) + threadIdx.x;
...
/** add_fibril_agent
 * Continuous fibril agent add agent function writes agent data to agent swap
 * @param agents xmachine_memory_fibril_list to add agents to 
...
__device__ void add_fibril_agent(xmachine_memory_fibril_list* agents, int id, float x, float y, float z, float fx, float fy, float fz){
...
__device__ void add_fibril_agent(xmachine_memory_fibril_list* agents, int id, float x, float y, float z, float fx, float fy, float fz){
    add_fibril_agent<DISCRETE_2D>(agents, id, x, y, z, fx, fy, fz);
}

/** reorder_fibril_agents
 * Continuous fibril agent areorder function used after key value pairs have been sorted
 ...
 */
__global__ void reorder_fibril_agents(unsigned int* values, xmachine_memory_fibril_list* unordered_agents, xmachine_memory_fibril_list* ordered_agents)
{
	int index = (blockIdx.x*blockDim.x) + threadIdx.x;
 ...
 *
 */
__global__ void GPUFLAME_outputedata(xmachine_memory_fibril_list* agents, xmachine_message_location_list* location_messages){
	
	//continuous agent: index is agent position in 1D agent list
	int index = (blockIdx.x * blockDim.x) + threadIdx.x;
  
    //For agents not using non partitioned message input check the agent bounds
    if (index >= d_xmachine_memory_fibril_count)
        return;
    

	//SoA to AoS - xmachine_memory_outputedata Coalesced memory read (arrays point to first item for agent index)
	xmachine_memory_fibril agent;
	...
	//FLAME function call
	int dead = !outputedata(&agent, location_messages	);
	

	//continuous agent: set reallocation flag
	agents->_scan_input[index]  = dead; 

	//AoS to SoA - xmachine_memory_outputedata Coalesced memory write (ignore arrays)
 ...
 *
 */
__global__ void GPUFLAME_inputdata(xmachine_memory_fibril_list* agents, xmachine_message_location_list* location_messages){
