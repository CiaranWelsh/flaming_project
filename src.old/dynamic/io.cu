...
void saveIterationData(char* outputpath, int iteration_number, xmachine_memory_fibril_list* h_fibrils_default, xmachine_memory_fibril_list* d_fibrils_default, int h_xmachine_memory_fibril_default_count)
{
	...
	cudaStatus = cudaMemcpy( h_fibrils_default, d_fibrils_default, sizeof(xmachine_memory_fibril_list), cudaMemcpyDeviceToHost);
	if (cudaStatus != cudaSuccess)
	{
		fprintf(stderr,"Error Copying fibril Agent default State Memory from GPU: %s\n", cudaGetErrorString(cudaStatus));
		exit(cudaStatus);
	}
	...
	//Write each fibril agent to xml
	for (int i=0; i<h_xmachine_memory_fibril_default_count; i++){
		fputs("<xagent>\n" , file);
		fputs("<name>fibril</name>\n", file);
        
		fputs("<id>", file);
        sprintf(data, "%d", h_fibrils_default->id[i]);
		fputs(data, file);
		fputs("</id>\n", file);
        
		fputs("<x>", file);
        sprintf(data, "%f", h_fibrils_default->x[i]);
		fputs(data, file);
		fputs("</x>\n", file);
        
		fputs("<y>", file);
        sprintf(data, "%f", h_fibrils_default->y[i]);
		fputs(data, file);
		fputs("</y>\n", file);
        
		fputs("<z>", file);
        sprintf(data, "%f", h_fibrils_default->z[i]);
		fputs(data, file);
		fputs("</z>\n", file);
        
		fputs("<fx>", file);
        sprintf(data, "%f", h_fibrils_default->fx[i]);
		fputs(data, file);
		fputs("</fx>\n", file);
        
		fputs("<fy>", file);
        sprintf(data, "%f", h_fibrils_default->fy[i]);
		fputs(data, file);
		fputs("</fy>\n", file);
        
		fputs("<fz>", file);
        sprintf(data, "%f", h_fibrils_default->fz[i]);
		fputs(data, file);
		fputs("</fz>\n", file);
...
void readInitialStates(char* inputpath, xmachine_memory_fibril_list* h_fibrils, int* h_xmachine_memory_fibril_count)
{
	...
	*h_xmachine_memory_fibril_count = 0;
	...
	in_fibril_fy = 0;
	in_fibril_fz = 0;
	//set all fibril values to 0
	//If this is not done then it will cause errors in emu mode where undefined memory is not 0
	for (int k=0; k<xmachine_memory_fibril_MAX; k++)
	{	
		h_fibrils->id[k] = 0;
		h_fibrils->x[k] = 0;
		h_fibrils->y[k] = 0;
	...
	while(reading==1)
	{
		...
		else if(c == '>')
		{
			...
			{
				if(strcmp(agentname, "fibril") == 0)
				{
					if (*h_xmachine_memory_fibril_count > xmachine_memory_fibril_MAX){
						printf("ERROR: MAX Buffer size (%i) for agent fibril exceeded whilst reading data\n", xmachine_memory_fibril_MAX);
						// Close the file and stop reading
						fclose(file);
						exit(EXIT_FAILURE);
					}
                    
					h_fibrils->id[*h_xmachine_memory_fibril_count] = fibril_id;
					h_fibrils->x[*h_xmachine_memory_fibril_count] = fibril_x;//Check maximum x value
                    if(agent_maximum.x < fibril_x)
                        agent_maximum.x = (float)fibril_x;
                    //Check minimum x value
                    if(agent_minimum.x > fibril_x)
                        agent_minimum.x = (float)fibril_x;
                    
					h_fibrils->y[*h_xmachine_memory_fibril_count] = fibril_y;//Check maximum y value
                    if(agent_maximum.y < fibril_y)
                        agent_maximum.y = (float)fibril_y;
                    //Check minimum y value
                    if(agent_minimum.y > fibril_y)
                        agent_minimum.y = (float)fibril_y;
                    
					h_fibrils->z[*h_xmachine_memory_fibril_count] = fibril_z;//Check maximum z value
                    if(agent_maximum.z < fibril_z)
                        agent_maximum.z = (float)fibril_z;
                    //Check minimum z value
                    if(agent_minimum.z > fibril_z)
                        agent_minimum.z = (float)fibril_z;
                    
					h_fibrils->fx[*h_xmachine_memory_fibril_count] = fibril_fx;
					h_fibrils->fy[*h_xmachine_memory_fibril_count] = fibril_fy;
					h_fibrils->fz[*h_xmachine_memory_fibril_count] = fibril_fz;
					(*h_xmachine_memory_fibril_count) ++;	
				}
				else
				{
			...
			if(strcmp(buffer, "id") == 0) in_fibril_id = 1;
			if(strcmp(buffer, "/id") == 0) in_fibril_id = 0;
			if(strcmp(buffer, "x") == 0) in_fibril_x = 1;
			if(strcmp(buffer, "/x") == 0) in_fibril_x = 0;
			if(strcmp(buffer, "y") == 0) in_fibril_y = 1;
			if(strcmp(buffer, "/y") == 0) in_fibril_y = 0;
			if(strcmp(buffer, "z") == 0) in_fibril_z = 1;
			if(strcmp(buffer, "/z") == 0) in_fibril_z = 0;
			if(strcmp(buffer, "fx") == 0) in_fibril_fx = 1;
			if(strcmp(buffer, "/fx") == 0) in_fibril_fx = 0;
			if(strcmp(buffer, "fy") == 0) in_fibril_fy = 1;
			if(strcmp(buffer, "/fy") == 0) in_fibril_fy = 0;
			if(strcmp(buffer, "fz") == 0) in_fibril_fz = 1;
			if(strcmp(buffer, "/fz") == 0) in_fibril_fz = 0;
		...
		else if(c == '<')
		{
			...
			{
				if(in_fibril_id){
                    fibril_id = (int) fpgu_strtol(buffer); 
                }
				if(in_fibril_x){
                    fibril_x = (float) fgpu_atof(buffer); 
                }
				if(in_fibril_y){
                    fibril_y = (float) fgpu_atof(buffer); 
                }
				if(in_fibril_z){
                    fibril_z = (float) fgpu_atof(buffer); 
                }
				if(in_fibril_fx){
                    fibril_fx = (float) fgpu_atof(buffer); 
                }
				if(in_fibril_fy){
                    fibril_fy = (float) fgpu_atof(buffer); 
                }
				if(in_fibril_fz){
                    fibril_fz = (float) fgpu_atof(buffer); 
                }
