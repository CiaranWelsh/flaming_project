...
cudaGraphicsResource_t fibril_default_cgr;
GLuint fibril_default_tbo;
GLuint fibril_default_displacementTex;
...
__global__ void output_fibril_agent_to_VBO(xmachine_memory_fibril_list* agents, glm::vec4* vbo, glm::vec3 centralise){
...
void initVisualisation()
{
	...
	// create TBO
	createTBO(&fibril_default_cgr, &fibril_default_tbo, &fibril_default_displacementTex, xmachine_memory_fibril_MAX * sizeof( glm::vec4));
...
void runCuda()
{
	...
	if (get_agent_fibril_default_count() > 0)
	{
		// map OpenGL buffer object for writing from CUDA
        size_t accessibleBufferSize = 0;
        gpuErrchk(cudaGraphicsMapResources(1, &fibril_default_cgr));
		gpuErrchk(cudaGraphicsResourceGetMappedPointer( (void**)&dptr, &accessibleBufferSize, fibril_default_cgr));
		//cuda block size
		tile_size = (int) ceil((float)get_agent_fibril_default_count()/threads_per_tile);
		...
		output_fibril_agent_to_VBO<<< grid, threads>>>(get_device_fibril_default_agents(), dptr, centralise);
		gpuErrchkLaunch();
		// unmap buffer object
        gpuErrchk(cudaGraphicsUnmapResources(1, &fibril_default_cgr));
	}
...
void display()
{
	...
	//Draw fibril Agents in default state
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_BUFFER_EXT, fibril_default_displacementTex);
	//loop
	for (int i=0; i< get_agent_fibril_default_count(); i++){
...
void close()
{
    ...
    deleteTBO( &fibril_default_cgr, &fibril_default_tbo);
