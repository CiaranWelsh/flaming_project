# The name of the MODEL (i.e. the name of the parent directory.)
MODEL := flaming_project
...
# the FLAME GPU MODEL projects, but this depnds upon several variables being
...
BIN_DIR := $(MODEL_BIN_DIR)/$(OS_BIN_DIR)
# Temporary directory for building
BUILD_DIR := $(MODEL_BUILD_DIR)/$(OS_BUILD_DIR)
...
# Paths to various source file directories for this MODEL
SRC_MODEL := src/model
SRC_VISUALISATION := src/visualisation
SRC_DYNAMIC := src/dynamic

# Full path to the XMLModelFile xml file for this model.
XML_MODEL_FILE:=$(SRC_MODEL)/model_file.xml

# Function Files(s) used within this model.
FUNCTIONS_FILES:= $(SRC_MODEL)/functions.c

# Include directories for console and visualisation builds
# If a custom visualisation is used, this may also need to include 
# $SRC_VISUALISATION because functions.c may include it.
INCLUDE_DIRS := \
	$(INCLUDE_DIR) \
	$(SRC_MODEL) \
	$(SRC_DYNAMIC) \
...
TARGET_VISUALISATION := $(BIN_DIR)/$(Mode_TYPE)_Visualisation/$(MODEL)$(BIN_EXT)
TARGET_CONSOLE := $(BIN_DIR)/$(Mode_TYPE)_Console/$(MODEL)$(BIN_EXT)
...
clean:
	@find $(MODEL_BUILD_DIR)/ -name '*$(OBJ_EXT)' -delete 2> /dev/null || true
	@find $(MODEL_BUILD_DIR)/ -name '*.cu$(OBJ_EXT)' -delete 2> /dev/null || true
...
clobber: clean
	...
	@find $(BIN_DIR)/ -name '$(MODEL)$(BIN_EXT)' -delete 2> /dev/null || true
