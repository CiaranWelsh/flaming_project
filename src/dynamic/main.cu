...
void runConsoleWithXMLOutput(int iterations, int outputFrequency){
	...
	for (int i=0; i< iterations || iterations == 0; i++)
	{
		...
		if((i+1) % outputFrequency == 0){
			saveIterationData(outputpath, i+1, get_host_A_moving_agents(), get_device_A_moving_agents(), get_agent_A_moving_count(),get_host_A_change_direction_agents(), get_device_A_change_direction_agents(), get_agent_A_change_direction_count(),get_host_A_get_going_again_agents(), get_device_A_get_going_again_agents(), get_agent_A_get_going_again_count());
	...
	if(iterations % outputFrequency != 0){
		saveIterationData(outputpath, iterations, get_host_A_moving_agents(), get_device_A_moving_agents(), get_agent_A_moving_count(),get_host_A_change_direction_agents(), get_device_A_change_direction_agents(), get_agent_A_change_direction_count(),get_host_A_get_going_again_agents(), get_device_A_get_going_again_agents(), get_agent_A_get_going_again_count());
		printf("Iteration %i Saved to XML\n", iterations);
	}
