...
__constant__ int d_xmachine_memory_A_moving_count;

__constant__ int d_xmachine_memory_A_change_direction_count;

__constant__ int d_xmachine_memory_A_get_going_again_count;
...
__device__ void add_A_agent(xmachine_memory_A_list* agents, int id, float x, float y, float z, float fx, float fy, float fz){
...
__device__ void add_A_agent(xmachine_memory_A_list* agents, int id, float x, float y, float z, float fx, float fy, float fz){
    add_A_agent<DISCRETE_2D>(agents, id, x, y, z, fx, fy, fz);
}
 ...
 *
 */
__global__ void GPUFLAME_move(xmachine_memory_A_list* agents){
 ...
 *
 */
__global__ void GPUFLAME_reverse_direction(xmachine_memory_A_list* agents){
	...
	//SoA to AoS - xmachine_memory_reverse_direction Coalesced memory read (arrays point to first item for agent index)
	xmachine_memory_A agent;
	...
	//FLAME function call
	int dead = !reverse_direction(&agent);
	

	//continuous agent: set reallocation flag
	agents->_scan_input[index]  = dead; 

	//AoS to SoA - xmachine_memory_reverse_direction Coalesced memory write (ignore arrays)
	agents->id[index] = agent.id;
	agents->x[index] = agent.x;
	agents->y[index] = agent.y;
	agents->z[index] = agent.z;
 ...
 *
 */
__global__ void GPUFLAME_resume_movement(xmachine_memory_A_list* agents){
	...
	//SoA to AoS - xmachine_memory_resume_movement Coalesced memory read (arrays point to first item for agent index)
	xmachine_memory_A agent;
	...
	//FLAME function call
	int dead = !resume_movement(&agent);
	

	//continuous agent: set reallocation flag
	agents->_scan_input[index]  = dead; 

	//AoS to SoA - xmachine_memory_resume_movement Coalesced memory write (ignore arrays)
	agents->id[index] = agent.id;
	agents->x[index] = agent.x;
	agents->y[index] = agent.y;
	agents->z[index] = agent.z;
