...
cudaGraphicsResource_t A_moving_cgr;
GLuint A_moving_tbo;
GLuint A_moving_displacementTex;

cudaGraphicsResource_t A_change_direction_cgr;
GLuint A_change_direction_tbo;
GLuint A_change_direction_displacementTex;

cudaGraphicsResource_t A_get_going_again_cgr;
GLuint A_get_going_again_tbo;
GLuint A_get_going_again_displacementTex;
...
void initVisualisation()
{
	...
	// create TBO
	createTBO(&A_moving_cgr, &A_moving_tbo, &A_moving_displacementTex, xmachine_memory_A_MAX * sizeof( glm::vec4));
	
	createTBO(&A_change_direction_cgr, &A_change_direction_tbo, &A_change_direction_displacementTex, xmachine_memory_A_MAX * sizeof( glm::vec4));
	
	createTBO(&A_get_going_again_cgr, &A_get_going_again_tbo, &A_get_going_again_displacementTex, xmachine_memory_A_MAX * sizeof( glm::vec4));
...
void runCuda()
{
	...
	if (get_agent_A_moving_count() > 0)
	{
		// map OpenGL buffer object for writing from CUDA
        size_t accessibleBufferSize = 0;
        gpuErrchk(cudaGraphicsMapResources(1, &A_moving_cgr));
		gpuErrchk(cudaGraphicsResourceGetMappedPointer( (void**)&dptr, &accessibleBufferSize, A_moving_cgr));
		//cuda block size
		tile_size = (int) ceil((float)get_agent_A_moving_count()/threads_per_tile);
		...
		output_A_agent_to_VBO<<< grid, threads>>>(get_device_A_moving_agents(), dptr, centralise);
		gpuErrchkLaunch();
		// unmap buffer object
        gpuErrchk(cudaGraphicsUnmapResources(1, &A_moving_cgr));
	}
	
	if (get_agent_A_change_direction_count() > 0)
	{
		// map OpenGL buffer object for writing from CUDA
        size_t accessibleBufferSize = 0;
        gpuErrchk(cudaGraphicsMapResources(1, &A_change_direction_cgr));
		gpuErrchk(cudaGraphicsResourceGetMappedPointer( (void**)&dptr, &accessibleBufferSize, A_change_direction_cgr));
		//cuda block size
		tile_size = (int) ceil((float)get_agent_A_change_direction_count()/threads_per_tile);
		grid = dim3(tile_size, 1, 1);
		threads = dim3(threads_per_tile, 1, 1);
		...
		output_A_agent_to_VBO<<< grid, threads>>>(get_device_A_change_direction_agents(), dptr, centralise);
		gpuErrchkLaunch();
		// unmap buffer object
        gpuErrchk(cudaGraphicsUnmapResources(1, &A_change_direction_cgr));
	}
	
	if (get_agent_A_get_going_again_count() > 0)
	{
		// map OpenGL buffer object for writing from CUDA
        size_t accessibleBufferSize = 0;
        gpuErrchk(cudaGraphicsMapResources(1, &A_get_going_again_cgr));
		gpuErrchk(cudaGraphicsResourceGetMappedPointer( (void**)&dptr, &accessibleBufferSize, A_get_going_again_cgr));
		//cuda block size
		tile_size = (int) ceil((float)get_agent_A_get_going_again_count()/threads_per_tile);
		grid = dim3(tile_size, 1, 1);
		threads = dim3(threads_per_tile, 1, 1);
		...
		output_A_agent_to_VBO<<< grid, threads>>>(get_device_A_get_going_again_agents(), dptr, centralise);
		gpuErrchkLaunch();
		// unmap buffer object
        gpuErrchk(cudaGraphicsUnmapResources(1, &A_get_going_again_cgr));
	}
...
void display()
{
	...
	//Draw A Agents in moving state
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_BUFFER_EXT, A_moving_displacementTex);
	//loop
	for (int i=0; i< get_agent_A_moving_count(); i++){
	...
	//Draw A Agents in change_direction state
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_BUFFER_EXT, A_change_direction_displacementTex);
	//loop
	for (int i=0; i< get_agent_A_change_direction_count(); i++){
		glVertexAttrib1f(vs_mapIndex, (float)i);
	...
	//Draw A Agents in get_going_again state
	glActiveTexture(GL_TEXTURE0);
	glBindTexture(GL_TEXTURE_BUFFER_EXT, A_get_going_again_displacementTex);
	//loop
	for (int i=0; i< get_agent_A_get_going_again_count(); i++){
		glVertexAttrib1f(vs_mapIndex, (float)i);
...
void close()
{
    ...
    deleteTBO( &A_moving_cgr, &A_moving_tbo);
    
    deleteTBO( &A_change_direction_cgr, &A_change_direction_tbo);
    
    deleteTBO( &A_get_going_again_cgr, &A_get_going_again_tbo);
